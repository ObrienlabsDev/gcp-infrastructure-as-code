...
gcloud compute networks subnets create $GKE_VPC_SN_NAME --project=$PROJECT_ID --description=$GKE_VPC_SN_NAME --range=10.0.0.0/16 --stack-type=IPV4_ONLY --network=$GKE_VPC_NAME --region=$GKE_REGION --enable-private-ip-google-access
...
#   --network "projects/$PROJECT_ID/global/networks/dev-man" --subnetwork "projects/$PROJECT_ID/regions/$GKE_REGION/subnetworks/$GKE_VPC_SN_NAME" \
