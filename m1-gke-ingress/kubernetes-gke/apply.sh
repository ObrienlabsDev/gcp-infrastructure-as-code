...
# manual
gcloud beta container --project "biometric-backend-gke-man-old" clusters create "dev2-man" --region "us-central1" --no-enable-basic-auth --cluster-version "1.34.1-gke.1829001" \
  ...
  --max-surge-upgrade 1 --max-unavailable-upgrade 0 --binauthz-evaluation-mode=DISABLED --no-enable-managed-prometheus --enable-secret-manager \
  --enable-secret-manager-rotation --secret-manager-rotation-interval "120s" --shielded-integrity-monitoring --no-shielded-secure-boot
